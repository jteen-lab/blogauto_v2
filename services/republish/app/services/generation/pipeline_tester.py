...
class PipelineTester:
    ...
    async def test_generate_content(
        ...
        return {
            ...
            "result": {
                "content_markdown": content_md, "content_html": "",
    ...
    async def test_add_internal_links(
        ...
        il_settings: Optional[dict] = None,
    ) -> dict:
        """Step 5: 내부링크 삽입 테스트
        ...
        Args:
            ...
            content: 마크다운 본문
            il_settings: UI에서 전달된 내부링크 설정 (선택, None이면 DB 설정 사용)
        """
        module = await self.db.get(Module, module_id)
        settings = module.settings or {} if module else {}
        db_il_settings = settings.get("internal_links", {})

        # UI 설정 우선, 없으면 DB 설정
        effective_il_settings = il_settings if il_settings else db_il_settings

        # InternalLinker에 전달할 module_settings 구성
        effective_module_settings = {"internal_links": effective_il_settings}

        start = time.time()
        result = await self.internal_linker.insert_links(
            content=content, blog_id=blog_id,
            current_title=current_title,
            module_settings=effective_module_settings,
        )
        elapsed = int(time.time() - start)
        ...
        return {
            ...
            "success": True,
            "result": {
                "content_with_links": result,
                "content_length": len(result),
                "generation_time_seconds": elapsed,
                "settings_used": {
                    "enabled": effective_il_settings.get("enabled", False),
                    "intro_count": effective_il_settings.get("intro_count"),
                    "body_count": effective_il_settings.get("body_count"),
                    "conclusion_count": effective_il_settings.get("conclusion_count"),
                    "source": "ui" if il_settings else "db",
                },
            },
        }
