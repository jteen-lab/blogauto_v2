...
네이버 검색 API를 사용하여 뉴스 제목을 수집합니다.
수집된 뉴스 제목은 키워드 추출 없이 그대로 임시제목 테이블에 저장됩니다.
...
# 뉴스 검색용 카테고리 키워드 (정보성 블로그에 적합)
NEWS_CATEGORIES = {
    "economy": ["경제", "주식", "부동산", "금융", "투자"],
    "it_tech": ["IT", "테크", "스마트폰", "인공지능", "AI"],
    "lifestyle": ["건강", "다이어트", "운동", "뷰티", "패션"],
    "travel": ["여행", "맛집", "카페", "핫플레이스"],
    "culture": ["영화", "드라마", "공연", "전시회"],
    "education": ["교육", "취업", "자격증", "공부법"],
}
...
class NaverNewsService:
    ...
    async def search_news(
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 429:
                        if attempt < self.MAX_RETRIES:
                            ...
                            logger.warning(
                                f"[NAVER_NEWS] Rate Limit - {delay:.1f}초 후 재시도 "
                    ...
                    if response.status_code != 200:
                        logger.warning(
                            f"[NAVER_NEWS] API 오류: {response.status_code}"
                        )
                        return {
                            "success": False,
                ...
                logger.warning(f"[NAVER_NEWS] 타임아웃 (시도 {attempt + 1})")
                if attempt < self.MAX_RETRIES:
                ...
                logger.error(f"[NAVER_NEWS] 검색 예외: {e}")
                return {
                    "success": False,
                    "error": str(e),
                    "items": []
    ...
    async def collect_news_titles(
        ...
        if use_random:
            return await self._collect_random_titles(categories, limit, delay_between_queries)

        logger.info(
            f"[NAVER_NEWS] 뉴스 제목 수집 시작 "
        ...
        for cat_key in categories:
            ...
            keywords = NEWS_CATEGORIES.get(cat_key, [cat_key])
            ...
            for keyword in keywords:
                ...
                if not result.get("success"):
                    logger.warning(
                        f"[NAVER_NEWS] '{keyword}' 검색 실패: {result.get('error')}"
                    )
                    continue
        ...
        logger.info(
            f"[NAVER_NEWS] 뉴스 제목 수집 완료: {collected_count}개"
        )
    ...
    async def _collect_random_titles(
        ...
        """
        logger.info(
            f"[NAVER_NEWS] 랜덤 수집 시작 "
            f"(카테고리 {len(categories)}개, limit={limit}, 20개씩)"
        )

        all_titles = []
        collected_count = 0
        used_keywords = set()  # 이미 사용한 키워드 추적
        max_attempts = limit * 2  # 무한 루프 방지
        attempts = 0
        ...
        while collected_count < limit and attempts < max_attempts:
            ...
            if not result.get("success"):
                logger.warning(
                    f"[NAVER_NEWS] 랜덤 수집 '{keyword}' 검색 실패"
                )
                continue
            ...
            logger.debug(
                f"[NAVER_NEWS] 랜덤 수집 '{keyword}': {sample_size}개 추가 "
        ...
        logger.info(
            f"[NAVER_NEWS] 랜덤 수집 완료: {collected_count}개 "
            f"(시도 {attempts}회)"
        )
    ...
    async def test_connection(self) -> Dict[str, Any]:
        """API 연결 테스트"""
        try:
            result = await self.search_news(query="뉴스", display=1)

            if result.get("success"):
                return {
                    "success": True,
                    "message": f"네이버 뉴스 API 연결 성공 (총 {result.get('total', 0)}건)"
                }
            else:
                return {
                    "success": False,
