/**
 * 키워드 모듈 스케줄 UI — 다른 모듈과 같은 방식.
 *
 * 실행 간격을 분 단위 숫자로만 받던 것을 수집 모듈과 같은 형태
 * (고정 시간 / 간격 + 활성 시간대)로 맞춘다. 성장 프로파일을 쓰지 않는
 * 모듈은 모두 이 UI 를 쓴다.
 *
 * 순서도: docs/flowcharts/keyword_module.md
 */
window.getKeywordScheduleTemplate = function () {
    return `
    <div class="border border-gray-200 rounded-lg p-3 space-y-4">
        ...
        <div class="flex gap-4">
            <label class="flex items-center cursor-pointer">
                <input type="radio" x-model="formData.keyword.schedule_mode" value="fixed_time"
                       class="text-amber-600 focus:ring-amber-500">
                <span class="ml-2 text-sm">고정 시간 (권장)</span>
            </label>
            <label class="flex items-center cursor-pointer">
                <input type="radio" x-model="formData.keyword.schedule_mode" value="interval"
                       class="text-amber-600 focus:ring-amber-500">
        ...
        <!-- 고정 시간 -->
        <div x-show="formData.keyword.schedule_mode === 'fixed_time'" class="p-3 bg-amber-50 rounded-lg">
            <label class="block text-sm font-medium text-gray-700 mb-2">실행 시간</label>
            <div class="flex flex-wrap gap-2 mb-3">
                <template x-for="(time, idx) in formData.keyword.fixed_times" :key="idx">
                    <span class="inline-flex items-center px-3 py-1 bg-amber-200 text-amber-900 rounded-full text-sm">
                        <span x-text="time"></span>
                        <button type="button" @click="removeKeywordTime(time)"
                                class="ml-2 text-amber-700 hover:text-amber-900">×</button>
                    </span>
                </template>
                <span x-show="!formData.keyword.fixed_times.length" class="text-xs text-gray-500 py-1">
                    아직 없습니다 — 시간을 추가하세요
                </span>
            </div>
            <div class="flex items-center gap-2">
                ...
                <button type="button" @click="addKeywordTime()"
        ...
        <!-- 간격 -->
        <div x-show="formData.keyword.schedule_mode === 'interval'" class="p-3 bg-amber-50 rounded-lg">
            <div class="flex items-center gap-2">
                <span class="text-gray-700 text-sm">실행 간격:</span>
                <input type="number" x-model.number="formData.keyword.interval_hours" min="1" max="24"
        ...
        <!-- 활성 시간대 -->
        <div x-show="formData.keyword.schedule_mode === 'interval'">
