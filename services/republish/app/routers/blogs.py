...
async def get_blogs(
    ...
    """블로그 목록 조회"""
    blog_service = BlogService(db)
    return await blog_service.get_user_blogs(current_user)
...
async def update_blog(
    ...
    logger.info(f"블로그 수정 API 요청 | 블로그ID={blog_id} | 사용자={current_user.id} | IP={client_ip}")
...
async def delete_blog(
    ...
    logger.info(f"블로그 삭제 API 요청 | 블로그ID={blog_id} | 사용자={current_user.id} | IP={client_ip}")
...
async def test_blog_connection(
    ...
    """블로그 연결 테스트"""
    logger.info(f"블로그 연결 테스트 API 요청 | 블로그ID={blog_id} | 사용자={current_user.id}")
...
async def blog_settings_page(
    ...
    try:
        ...
        logger.error(f"블로그 설정 페이지 오류 | 블로그ID={blog_id} | 사용자={current_user.id} | 오류={e}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
