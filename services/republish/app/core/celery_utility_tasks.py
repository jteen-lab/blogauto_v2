...
# ==========================================================================
# 제목 카테고리 재분류(대량) — 프록시 타임아웃 방지용 백그라운드 태스크
# ==========================================================================

async def _async_reclassify(
    target: str, force_all: bool, user_id: int,
) -> dict:
    """재분류 비동기 본문(celery 세션 사용)."""
    from app.core.celery_async_bridge import celery_db_session
    from app.services.title_reclassify_service import reclassify_titles

    async with celery_db_session() as db:
        return await reclassify_titles(
...
def reclassify_titles_task(
    ...
    Args:
        ...
        force_all: True면 전체, False면 미분류만
        user_id: 키워드 소유 사용자

    Returns:
        dict: {"success": bool, "total": int, "matched": int}
    """
    logger.info(
        "[TASK:RECLASSIFY] 시작 | target=%s | force_all=%s",
        target, force_all,
    )
    try:
        result = run_async(_async_reclassify(target, force_all, user_id))
        logger.info(
            "[TASK:RECLASSIFY] 완료 | 전체 %s개 중 %s개 매칭",
            result.get("total"), result.get("matched"),
        )
        return result
    except Exception as exc:
        logger.error("[TASK:RECLASSIFY] 예외 | %s", exc)
        raise
