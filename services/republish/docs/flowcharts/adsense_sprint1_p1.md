...
- **완료**: F2(저자성) — `author_profile` 필드 + 저장 API + About 페이지 반영
  + 개별 글 본문 바이라인/편집감독 문구 주입(`generator.py` 6.8단계,
  `author_signal_injector.py` 신규). **옵트인**: `author_profile.name`이
  설정된 블로그만 영향받으므로 기존 운영 중인 블로그(전부 미설정)는 동작
  변화 없음. 단위테스트 4종 통과.
- **완료**: 관리자 UI — 블로그 설정에 "애드센스" 탭 추가
  (`_tab_adsense.html`) — 필수 페이지 생성 버튼/상태 표시, 저자 프로필
  입력/저장 폼.
- **미착수**: F3(카테고리 메뉴 자동 구성) — Blogger 템플릿 PATCH/WordPress
  메뉴 REST는 플랫폼별 리스크가 크고 실제 블로그로 검증 없이는 발행 중인
  템플릿을 깨뜨릴 위험이 있어 이번 세션에서도 보류. 다음 세션 과제.
