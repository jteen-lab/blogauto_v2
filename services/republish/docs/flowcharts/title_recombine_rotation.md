...
## 같은 키워드에 템플릿이 여럿이면 (2026-09-17)

`이사+견적` 을 템플릿 1 과 4 에 함께 걸면 둘 다 조건을 맞춘다. 예전에는
**제목 해시**로 갈랐다 — 같은 제목은 늘 같은 템플릿이고, 어느 글이 어디로
갈지 사람이 알 수 없었다.

이제 **후보 중에서 무작위로** 고른다. 같은 키워드에 템플릿을 둘 이상 두는
이유가 글마다 다른 구조를 쓰기 위함이라, 고정이면 그 뜻이 살지 않는다.

```mermaid
flowchart TD
    A[제목] --> B[키워드가 맞는 템플릿만 남긴다]
    B --> C{몇 개 남았나}
    C -->|하나| D[그것을 쓴다]
    C -->|여럿| E[무작위로 하나]
    C -->|없음| F[키워드를 비운 템플릿<br/>그것도 없으면 기본 프롬프트]
```

**키워드 매칭은 그대로다.** 청소 글에 비용 템플릿이 걸리는 일은 없다 —
무작위는 **맞는 것들 사이에서만** 돈다.
