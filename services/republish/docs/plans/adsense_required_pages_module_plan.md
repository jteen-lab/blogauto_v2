...
### 2026-08-20 사용자 보고 3건 — 진단 결과
1. **"템플릿·문체 프리셋이 상단 값으로 저장된다"** → 저장은 정상, **표시 버그**.
   `<select>` 안 옵션을 `x-for`로 그리면 목록이 비동기로 채워질 때 x-model 값이
   DOM에 반영되지 않아 첫 옵션이 선택된 것처럼 보인다(디자인 갤러리는 클릭 즉시
   대입이라 무사했음). 두 선택도 버튼 목록으로 교체(20335a8).
   서버 DB 확인: module 193 = with_subject/friendly/dark, 194 = simple/concise/brand_blue.
2. **"실행해도 설정이 반영되지 않는다"** → 마지막 실행(19:21:10)이 모듈 마지막
   저장(19:25:13)보다 **앞섰다**. 서버에서 현재 설정으로 렌더 경로를 태우면
   friendly가 정확히 적용되고, Tally 폼에도 with_subject의 '제목' 필드가 들어가 있다.
   재실행하면 반영된다. 혼동 방지로 실행 결과에 적용 설정 표시(3f87532).
3. **"문의폼 디자인이 기본으로 생성된다"** → **실제 버그 발견**. styles=None이면
   `settings`를 아예 안 보내 Tally에 옛 디자인이 남았다(=디자인 '기본' 되돌리기 불가).
   `apply_styles`로 명시 초기화(105ce33).

#### Tally API 실측(테스트 폼 생성→확인→삭제)
- `settings.styles`는 **객체**로 POST/PATCH 모두 정상 저장된다(문자열로 보내면 400
  `"styles" must be of type object`). 기존 코드 형식 자체는 옳았다.
- `theme: DARK`, `font: {provider, family}`도 그대로 저장됨(문서 미기재지만 동작).
- `settings.styles: null` 전송으로 외형 초기화 가능 — 되돌리기 구현 근거.
