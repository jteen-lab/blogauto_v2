...
### 5-6. 벤치마킹 결과
별도 문서로 정리했다 → **`keyword_management_benchmark.md`**
(국내 키워드 도구/자동화 프로그램, 해외 대량 발행 툴, 클러스터링·프로그래매틱 SEO·query fan-out).
핵심 세 가지:
- 판정 축을 누적 문서수 → **월간 발행량**으로 바꿔야 하고, 검색량 **상한**도 필요하다.
- 생산 단위를 키워드 1개 → **클러스터(필러+서브 N편)** 로 올려야 대량 생성이 성립한다.
- **수요는 네이버로 재는데 발행은 구글(워드프레스/블로거)** 이다. 이미 보유한
  `google_keyword_planner_service` / `google_trends_service` / `search_visibility` 가 미연결이다.
