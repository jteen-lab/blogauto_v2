...
| `advisory` | **기계 검사 불가** | "성의 있게 작성" | 허위 설치 유도 금지 |

### 검증

의료 가이드라인의 규칙을 전부 분해했다.

```
의료 가이드라인 규칙 34건 → 검사 유형 13종
다른 업종 규칙 10건(대부업·앱설치·쇼핑·보험) → 새로 필요한 유형: format 1종
최종 14종
```

**업종이 바뀌어도 유형은 거의 늘지 않는다.** 늘어나는 것은 유형이 아니라
그 안의 값이다. 값은 열려 있고 유형은 닫혀 있다.

### 스키마

```
[고정 코어]  모든 CPA 오퍼에 반드시 있다
  network / offer_id / name / advertiser / vertical
  landing_url / subid_param
  conversion  { action, fields[], reject_reasons[] }
  ftc_notice  공정위 대가성 문구 — 법이라 업종 무관 공통
  raw_text    원문 전체
  recheck_due / status

[개방형 규칙]  0~N개. 유형은 위 14종 중 하나
  { type, target, value, source_quote, severity }
                              ↑
                    원문의 어느 문장에서 나왔는지
```

`source_quote` 를 반드시 남긴다. 규칙이 왜 생겼는지 원문으로 되짚지 못하면
사람이 확인할 수 없다.
